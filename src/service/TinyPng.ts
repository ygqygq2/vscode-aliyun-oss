  ...
  public static async uploadFileByApi(ossPath: string, localFilePath: string, cb: Function) {
    fs.stat(localFilePath, async (err: any, stats: any) => {
      ...
      if (isSupportTinyPng(localFilePath, stats.size)) {
        ...
        const res = await AliOss.ossUploadFile(ossPath, localFilePath);
  ...
  public static async uploadOssFile(ossPath: string, buff: Buffer, cb: Function) {
    ...
    const req = https.request(options, function (res: any) {
      res.on('data', (buf: any) => {
        ...
        const outputFileSize = getSizeString(obj.output.size);
        // 读取TinyPng图片压缩数据
        const options = new URL(obj.output.url);
        const reqTinyPng = https.request(options, (res: any) => {
          let data = '';
